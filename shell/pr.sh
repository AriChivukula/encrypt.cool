...
bash shell/build.sh
echo "no" | terraform init -backend-config="bucket=${TF_VAR_DOMAIN}" -backend-config="key=tfstate/PR_${TF_VAR_BRANCH}.tfstate" infra/pr
terraform apply -auto-approve infra/pr
echo "no" | terraform init -backend-config="bucket=${TF_VAR_DOMAIN}" -backend-config="key=tfstate/trigger.tfstate" infra/trigger
